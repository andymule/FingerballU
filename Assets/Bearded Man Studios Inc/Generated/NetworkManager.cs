...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] FlixNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is FlixNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (FlixNetworkObject.Length > 0 && FlixNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(FlixNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<FlixBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateFlix instead, its shorter and easier to type out ;)")]
		public FlixBehavior InstantiateFlixNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(FlixNetworkObject[index]);
			var netBehavior = go.GetComponent<FlixBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<FlixBehavior>().networkObject = (FlixNetworkObject)obj;
		...
		/// <summary>
		/// Instantiate an instance of Flix
		/// </summary>
		/// <returns>
		/// A local instance of FlixBehavior
		/// </returns>
		/// <param name="index">The index of the Flix prefab in the NetworkManager to Instantiate</param>
		/// <param name="position">Optional parameter which defines the position of the created GameObject</param>
		/// <param name="rotation">Optional parameter which defines the rotation of the created GameObject</param>
		/// <param name="sendTransform">Optional Parameter to send transform data to other connected clients on Instantiation</param>
		public FlixBehavior InstantiateFlix(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(FlixNetworkObject[index]);
			var netBehavior = go.GetComponent<FlixBehavior>();
